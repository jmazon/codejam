...
-- imports for Data.FingerTree.PSQueue
...
main = do
  ...
  forM_ [1..t] $ \tn -> do
    ...
    let s = listArray (bounds b)
        ...
        h = psqFromList $ map (uncurry (:->) . second negate) (assocs s)
        r = map (head &&& length) $ group $ unfoldr cut (h)
    putStrLn $ "Case #" ++ show tn ++ ": " ++ show (length r)
    mapM_ (putStrLn . \(i,j) -> show i ++ " " ++ show j) r

for :: [a] -> (a -> b) -> [b]
for = flip map

cut :: PSQ (Int,Int) Int -> Maybe (Int,PSQ (Int,Int) Int)
cut (h) | psqNull h  = Nothing
        | otherwise = Just (-s1,h')
    where Just ((i1,j1) :-> s1) = psqFindMin h
          h' = foldr (psqUpdateWithKey u) h
...
instance (Ord a) => Monoid (Prio k a)
  where mempty = PMax
instance Ord a => Semigroup (Prio k a) where
        PMax <> y = y
        x <> PMax = x
        a@(Prio (_ :-> x)) <> b@(Prio (_ :-> y)) = if x <= y then a else b
...
leqKey :: Ord a => a -> Key a -> Bool
leqKey _ NoKey = False
leqKey k (Key k') = k <= k'
...
instance (Semigroup (KPS k p),Ord p) => Monoid (KPS k p)
  where mempty = KPS mempty mempty mempty
instance Ord p => Semigroup (KPS k p) where
        (KPS k p s) <> (KPS k' p' s') = KPS (k <> k')
                                            (p <> p')
                                            (s <> s')


instance (Ord k, Ord p) => Measured (KPS k p) (Binding k p)
  where measure a@(k :-> _) = KPS (Key k) (Prio a) (Sum 1)
...
psqNull :: (Ord k, Ord p) => PSQ k p -> Bool
psqNull (PSQ q) = ftNull q

-- | O(log n). Determine if a key is in the queue, and its priority.
lookup :: (Ord k, Ord p) => k -> PSQ k p -> Maybe p
lookup k (PSQ q) = let (_,v) = ftSplit ((>= Key k) . kpsKey) q
                    in case ftViewl v of
                         EmptyL -> Nothing
                         (k' :-> p) :< _ | k == k'   -> Just p
                                        | otherwise -> Nothing
...
singleton k p = PSQ (ftSingleton (k :-> p))
...
alter f k (PSQ q) =
  PSQ $ let (u,v) = ftSplit (leqKey k . kpsKey) q
         in case ftViewl v of
...
-- the key as a parameter.
psqUpdateWithKey :: (Ord k, Ord p) => (k -> p -> Maybe p) -> k -> PSQ k p -> PSQ k p
psqUpdateWithKey f k = update (f k) k

-- | O(n). Flatten a queue into a list of bindings.
psqToList :: (Ord k, Ord p) => PSQ k p -> [Binding k p]
psqToList (PSQ q) = F.toList q

-- | O(n). Extract the list of keys of a queue.
keys :: (Ord k, Ord p) => PSQ k p -> [k]
keys = map (\(k :-> _) -> k) . psqToList

-- | O(n log n). Construct a queue from a list of bindings.
psqFromList :: (Ord k, Ord p) => [Binding k p] -> PSQ k p
psqFromList = PSQ . ftFromList . sort
...
minView (PSQ q) =
  let minPrio = kpsPrio . measure $ q
      (u,v) = ftSplit ((== minPrio) . kpsPrio) q
   in case ftViewl v of
        EmptyL -> Nothing
        ((k :-> p) :< v') -> Just (k :-> p, PSQ (u >< v'))

-- | O(1). Find the binding with minimum priority in a queue.
psqFindMin :: (Ord k, Ord p) => PSQ k p -> Maybe (Binding k p)
psqFindMin q = case kpsPrio . measure $ q of
                    PMax   -> Nothing
                    Prio b -> Just b
...
range (l,u) (PSQ q) = PSQ (fst . ftSplit ((> Key u) . kpsKey) . snd . ftSplit ((>= Key l) . kpsKey) $ q)
...
atMost p (PSQ q) =
  let less (Prio (_ :-> p')) = p' < p
      (_,v) = ftSplit (less . kpsPrio) q
   in case ftViewl v of
        EmptyL    -> []
        (b :< v') -> b : atMost p (PSQ v')

-- | Right fold over the list of bindings in a queue. 
psqFoldr :: (Ord k, Ord p) => (Binding k p -> b -> b) -> b -> PSQ k p -> b
psqFoldr f z = Prelude.foldr f z . psqToList

-- | Left fold over the list of bindings in a queue.
foldl :: (Ord k, Ord p) => (b -> Binding k p -> b) -> b -> PSQ k p -> b
foldl f z = Prelude.foldl f z . psqToList
...
ftFromList :: (Measured v a) => [a] -> FingerTree v a
ftFromList = foldr (<|) Empty
