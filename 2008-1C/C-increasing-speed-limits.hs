...
data Elem = Elem { eFst :: Int, eSnd :: Sum Int } deriving Show
instance Measured (Sum Int) Elem where measure = eSnd
type F = FingerTree (Sum Int) Elem

solve :: [Int] -> F
solve = foldl' f ftEmpty
    where f q x = l >< ( (Elem x (v `mappend` (Sum 1) `mappend` (measure l))) <| r' )
              where (l,r) = ftSplit (>= Sum x) q
                    (v,r') = case ftViewl r of
                               EmptyL -> (mempty,r)
                               ((Elem k v'):<r'') -> if k==x then (v',r'') else (mempty,r)

rnd :: (Int,Int,Int,Int,Int,[Int]) -> [Int]
...
main :: IO ()
main = interact $ unlines . zipWith c [1 :: Int ..] . map (solve . rnd) . parse . map read . tail . words
