...
parse [] = []
parse (_n:_m:ws) = (n,ts,rs) : parse ws' where
  n = read _n
  (ms,ws') = splitAt (3 * read _m) ws
  (ts,rs) = foldr readModel ([],[]) (chunksOf 3 ms)
  readModel [t,_i,_j] (rs,bs) = (rs',bs') where
    rs' = if t == "+" then rs else pos : rs
...
solve n rooks bishops = (score,rooks' ++ bishops' ++ trendies) where
  newRooks = solveRooks rooks
  newBishops = map fromDiags $ solveBishops $ map toDiags bishops
  toDiags (i,j) = (i - j, i + j - n - 1 :: Int)
  ...
  trendies = ((,) "o") <$> intersect newRooks bishops ++
                           intersect newBishops rooks ++
                           intersect newRooks newBishops

  solveRooks rs = zip is' js' where
    (is,js) = unzip rs
    is' = [1..n] \\ is
    js' = [1..n] \\ js

  solveBishops bs = go is' js' where
    (is,js) = unzip bs
