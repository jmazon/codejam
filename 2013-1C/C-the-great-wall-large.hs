...
-- imports for FingerTree/PriorityQueue
import GHC.Generics
...
data Wall = Wall { wallWest :: !Int, wallEast :: !Int, wallStrength :: !Int }
            deriving Show
instance Semigroup Wall where (Wall aw ae as) <> (Wall bw be bs) =
...
main = do
  ...
  forM_ [1..t] $ \i -> do
    n <- readLn
    as <- liftM (foldl' enqueue pqEmpty) $ replicateM n $ do
      [d,c,w,e,s,dd,dp,ds] <- liftM (map read . words) getLine
      return $ Attack d c (2*w) (2*e) s dd (2*dp) ds
    let wall = ftSingleton $ Wall minBound maxBound 0
    putStrLn $ "Case #" ++ show i ++ ": " ++ show (solve wall as 0)

solve :: FingerTree Wall Wall -> PQueue Int Attack -> Int -> Int
solve walls attacks0 c
    | pqNull attacks0 = c
    | otherwise      = solve walls' attacks0' $! c + successes
    where Just (firstAttack,_) = pqMinView attacks0
          day = attackDay firstAttack
          (dayAttacks,restAttacks) = getAttacks [] attacks0
          getAttacks r attacks
              | pqNull attacks || attackDay a > day = (r,attacks)
              | otherwise                           = getAttacks (a:r) attacks'
              where Just (a,attacks') = pqMinView attacks
          daySuccess a = fst (wLevel walls a)
          successes = length $ filter daySuccess dayAttacks
          walls' = foldl' (\w -> snd . wLevel w) walls dayAttacks
          attacks0' = foldl' enqueue restAttacks $
                      catMaybes $ map nextAttack dayAttacks

enqueue :: PQueue Int Attack -> Attack -> PQueue Int Attack
enqueue q a = pqInsert (attackDay a) a q

nextAttack :: Attack -> Maybe Attack
nextAttack (Attack d c w e s dd dp ds)
    | c <= 1    = Nothing
    | otherwise = Just (Attack (d+dd) (c-1) (w+dp) (e+dp) (s+ds) dd dp ds)

wLevel :: FingerTree Wall Wall -> Attack -> (Bool,FingerTree Wall Wall)
wLevel w0 (Attack _ _ aw ae s _ _ _) = ( wallStrength (measure middleWalls) < s,
                                         westWalls >< leveledWalls >< eastWalls )
    where (westWalls,nonWestWalls) = ftSplit ((>= aw) . wallEast) w0
          (middleWalls',eastWalls') = ftSplit ((>= ae) . wallEast) nonWestWalls
          borderEast :< eastWalls = ftViewl eastWalls'
...
compress :: [Wall] -> [Wall]
compress (a@(Wall aw _ae as) : ws @ (Wall _bw be bs : ws'))
...
-- | 'empty' and '><'.
instance (Measured v a) => Monoid (FingerTree v a) where mempty = ftEmpty
...
ftFmap' :: (Measured v1 a1, Measured v2 a2) =>
...
ftEmpty :: Measured v a => FingerTree v a
ftEmpty = Empty

-- | /O(1)/. A singleton sequence.
ftSingleton :: Measured v a => a -> FingerTree v a
ftSingleton = Single
...
ftNull :: FingerTree v a -> Bool
ftNull Empty = True
ftNull _ = False

-- | /O(1)/. Analyse the left end of a sequence.
ftViewl :: (Measured v a) => FingerTree v a -> ViewL (FingerTree v) a
ftViewl Empty                     =  EmptyL
ftViewl (Single x)                =  x :< Empty
ftViewl (Deep _ (One x) m sf)     =  x :< rotL m sf
ftViewl (Deep _ pr m sf)          =  lheadDigit pr :< deep (ltailDigit pr) m sf

rotL :: (Measured v a) => FingerTree v (Node v a) -> Digit a -> FingerTree v a
rotL m sf      =   case ftViewl m of
...
ftSplit ::  (Measured v a) =>
      (v -> Bool) -> FingerTree v a -> (FingerTree v a, FingerTree v a)
ftSplit _ Empty  =  (Empty, Empty)
ftSplit p xs
  | p (measure xs) =  (l, x <| r)
  | otherwise   =  (xs, Empty)
  where
...
data Entry k v = Entry k v deriving (Generic)
...
data Prio k v = NoPrio | Prio k v deriving (Generic)
...
pqEmpty = PQueue ftEmpty

-- | /O(1)/. A singleton priority queue.
pqSingleton :: Ord k => k -> v -> PQueue k v
pqSingleton k v = PQueue (ftSingleton (Entry k v))
...
pqInsert :: Ord k => k -> v -> PQueue k v -> PQueue k v
pqInsert k v (PQueue q) = PQueue (Entry k v <| q)
...
pqFromList = foldr (uncurry pqInsert) pqEmpty

-- | /O(1)/. Is this the empty priority queue?
pqNull :: Ord k => PQueue k v -> Bool
pqNull (PQueue q) = ftNull q
...
--
pqMinView :: Ord k => PQueue k v -> Maybe (v, PQueue k v)
pqMinView q = fmap (snd *** id) (minViewWithKey q)
...
minViewWithKey (PQueue q)
  | ftNull q = Nothing
  | otherwise = Just ((k, v), case ftViewl r of
    _ :< r' -> PQueue (l >< r')
    _ -> error "can't happen")
  where
    Prio k v = measure q
    (l, r) = ftSplit (below k) q
