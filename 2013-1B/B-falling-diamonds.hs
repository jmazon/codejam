...
solve (n,x,y) = go n 0
    where ring = (abs x + abs y) `div` 2
          go n i | n <= 0      = 0
                 ...
                 | n > y + 2*i = 1
                 | n > y       = hackDiv (sum [ choose n j | j <- [0..n-y-1] ]) (2^n)
                 | otherwise   = 0
choose = curry (a!)
    where a = array ((0,0),(1414,1414))
                    [ ((i,j),f i j) | i <- [0..1414], j <- [0..1414] ]
          f n p | p < 0 || p > n = 0
          f 0 _                  = 1
          f _ 0                  = 1
          f n p                  = a!(n-1,p-1) + a!(n-1,p)
hackDiv n d | d > 2^32  = hackDiv (n `div` 2) (d `div` 2)
            | otherwise = fromIntegral n / fromIntegral d
